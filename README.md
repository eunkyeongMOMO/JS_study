...
쿠키 : 사용자가 특정한 웹사이트에 방문 할 때, 사용자 컴퓨터에 저장하는 기록 파일이다.
서버의 자원을 전혀, 사용하지 않는다 
예시) 아이디와 비밀번호 저장

세션 : 한명의 사용자(브라우저)의 상태를 유지하는 기술이다.
서버가 클라이언트(브라우저)에게 고유한 Session ID를 부여하면, 클라이언트는 접속 할 때마다, Session ID와 함께 요청한다.
예시) 웹사이트에 한번 로그인하면 다른 페이지로 이동해도 계속 로그인 상태가 유지
Session ID를 다른 클라이언트에게 탈취 당헐수 있기때문에 회원정보 수정릏 할때에는 또 한번 비밀번호를 입력하여 토큰을 발급, 
탈취당해 개인정보가 바뀌거나 하는걸 방지하도록 해야한다.

서버에서 가지고 있는 객체로 주로 특정 사용자의 로그인정보를 유지하기 위해 사용된다.
> 예) 우리가 웹사이트에 로그인한 뒤에 서버에서는 세션ID에 따른 회원 ID 정보를 기록한다.
클라이언트는 해당 세션을 계속 유지한다. 예를들어 메일함에 접속할 때도 세션 ID를 서버에 전송한다.
다시 말해 **세션은 자신이 누구인지를 서버에 알려주는 역할**

1. 장점
 클라이언트에게는 세션ID를 제공하고 회원에대한 중요한 정보를 서버가 가지고 있다.
...
2. 단점
 ...
 웹서버애 세션정보를 기록하고 았어야 임으로, 접속자가 많ㅇ을때, 서버에 메노리 부하가 존재할 수 있다.
 
### HTTP  [hyperText tranfer Protocol]

1. 웹상에서 데이터를 주고받기 위한 프로토콜
2. 웹문서를 주고받기 위라여 사용할 수 있다.
3. 웹뿐만 아니라 모바일앱, 게임개발에서도 다양한 목적으로 사용됨

**HTTP Method**

클라이언트 요청의 목적에 따라 적절한 HTTP 메서드를 사용한다.

HTTP 메소드 | 설명 | 사용예시
--|--|--
GET |  데이터 조회를 요청한다. | 특정페이지 접속, 정보검색
POST | 데이터 생성을 요청한다. | 회원가입, 글쓰기
PUT | 데이터 수정을 요청한다 | 회원 정보 수정
DELETE | 데이터 삭제를 요청한다, | 회원 정보 삭제

특정한 웹사이트에 접속하면 기본적으로 get 방식으로 호출을 진행한다.

**상태코드 (STATUS CODE)**를 이용해 본인의 요청에 대한 결과를 응답 받을수 있다.
> 200의 경우 응답의 성공 [코드번호에 따라 에러가 다름으로 확인필요]
웹사이트는 대게 HTML,CSS,JAVASCRIPT코드를 반환하며 웹브라우저는 이를 화면에 출력한다.

**HTTP 상태관리와 세션**

1. HTTP는 상태를 저장하지 않는다 [stateless]
2. 글라이언트는 http로 서버에 연결한 뒤에 **응답을 받으면 연결을 끊어버린다**
> 서버입장에서 접속유지에 대한 요구가 적어 불특정 다수를 대상으로하는 서비스에 적합하다.
예시로ㅡ 상품확인 -> 장바구니 -> 결제의 과정이 시스템적으로 상태정보로  기록되지 않는다. 
하지만! 세션을 이용해 원하는 기능을 수행하도록한다.

**keep Alive 기능**
1. http 1.1 버전부터 기본으로 지원한다. 하나의 웹사이트에 방문하면 수십개의 파일(이미지, css, html, js)를 제공한다. TCP 통신 과정에서 연결수행, 연결 해체과정에서 리소스가 많이 소요되기때문에 이미 연결되어 있는 TCP연결을 재사용 하는 것.
kepp Alive는 이런 파일을 하나씩 받기위하여 매번 연결을 맺고 끊는것을 방지한다. 
